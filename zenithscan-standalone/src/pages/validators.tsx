...
interface Validator {
  address: string; name: string; rank: number; status: string;
  stake: string; delegatedStake: string; uptime: number; region: string;
}
        ...
        {[
          ...
          <Card key={i} className="rounded-2xl border-border/60 bg-white shadow-sm">
            <CardContent className="p-4">
              ...
              <div className={`text-xl font-bold font-mono ${stat.color ?? "text-foreground"}`}>{stat.value}</div>
            </CardContent>
          </Card>
        ))}
      </div>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 px-6 text-center gap-5">
              <div className="w-20 h-20 rounded-full flex items-center justify-center" style={{ background: "hsl(200 80% 95%)" }}>
                ...
                <tbody>
                  {validators!.map((v) => (
                    <tr key={v.address} className="border-b border-border/40 last:border-0 hover:bg-muted/20 transition-colors" data-testid={`validator-row-${v.rank}`}>
                      <td className="py-3 pl-4 pr-3"><span className="font-mono text-xs text-muted-foreground">#{v.rank}</span></td>
                      <td className="py-3 pr-4">
                        <Link href={`/address/${v.address}`} className="font-medium text-sm hover:text-primary">{v.name}</Link>
                      ...
                      <td className="py-3 pr-4 text-right hidden lg:table-cell">
                        <span className={`font-mono text-xs ${parseFloat(v.uptime.toString()) >= 99 ? "text-green-500" : "text-yellow-500"}`}>{v.uptime}%</span>
                      </td>
