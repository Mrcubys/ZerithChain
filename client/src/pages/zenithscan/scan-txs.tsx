...
export default function ScanTxs() {
  ...
  const { data: txs, isLoading } = useQuery<Transaction[]>({
    queryKey: ["/api/transactions", "scan-all"],
    queryFn: async () => {
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="relative w-full sm:w-64">
          ...
          <Input
            ...
            data-testid="input-scan-txs-filter"
          />
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {isLoading
                ...
                : filtered.map((tx) => (
                  <tr key={tx.hash} className="border-b border-border/40 last:border-0 hover:bg-muted/20 transition-colors" data-testid={`scan-tx-${tx.hash.slice(0, 8)}`}>
                    <td className="py-3 px-4">
                      <Link href={`/scan/tx/${tx.hash}`} className="font-mono text-xs text-primary hover:underline" data-testid={`link-tx-${tx.hash.slice(0, 8)}`}>
                        {shortHash(tx.hash)}
                      </Link>
                    </td>
                    ...
                    <td className="py-3 pr-4 hidden md:table-cell">
                      <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                        <Link href={`/scan/address/${tx.from}`} className="font-mono hover:text-foreground transition-colors">{shortAddress(tx.from)}</Link>
                        <ArrowRight className="w-3 h-3 flex-shrink-0" />
                        <Link href={`/scan/address/${tx.to}`} className="font-mono hover:text-foreground transition-colors">{shortAddress(tx.to)}</Link>
                      </div>
                    </td>
                    ...
                    <td className="py-3 pr-4 text-right">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-sm text-xs font-medium border capitalize ${statusBadgeClass(tx.status)}`}>
                        {tx.status}
                      </span>
                    </td>
                    <td className="py-3 pr-4 text-right">
