...
export default function ScanHome() {
  ...
  const { data: txs, isLoading: txsLoading } = useQuery<Transaction[]>({
    queryKey: ["/api/transactions", "scan-home"],
    queryFn: async () => {
  ...
  const { data: netStatus } = useQuery<{ tps: number; blockTime: number; latestBlock: number; totalValidators: number; totalTransactions: number }>({
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    const q = query.trim();
    if (!q) return;
    if (/^\d+$/.test(q)) navigate(`/scan/block/${q}`);
    else if (q.length === 64 && /^[0-9A-Fa-f]+$/.test(q)) navigate(`/scan/tx/${q}`);
    else navigate(`/scan/address/${q}`);
    setQuery("");
  };
    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-br from-primary/10 via-primary/5 to-transparent rounded-2xl p-6 border border-primary/15">
        ...
        <form onSubmit={handleSearch} className="flex gap-2 max-w-xl">
          ...
          <Button type="submit" className="rounded-xl px-5" data-testid="button-home-search">
            Search
          </Button>
        </form>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <Card className="rounded-2xl border-border/60 bg-white shadow-sm">
          <CardHeader className="pb-2 flex flex-row items-center justify-between">
            ...
            <Link href="/scan/blocks" className="text-xs text-primary hover:underline font-medium" data-testid="link-view-all-blocks">
              View all →
            </Link>
          </CardHeader>
          <CardContent className="pt-0 p-0">
            {blocksLoading
              ...
              : blocks?.map((block) => (
                <div key={block.height} className="flex items-center gap-3 px-4 py-3 border-b border-border/40 last:border-0 hover:bg-muted/30 transition-colors" data-testid={`scan-block-row-${block.height}`}>
                  <div className="w-9 h-9 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0">
                    <Layers className="w-4 h-4 text-primary" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <Link href={`/scan/block/${block.height}`} className="font-mono font-semibold text-sm hover:text-primary transition-colors" data-testid={`link-scan-block-${block.height}`}>
        ...
        <Card className="rounded-2xl border-border/60 bg-white shadow-sm">
          <CardHeader className="pb-2 flex flex-row items-center justify-between">
            ...
            <Link href="/scan/txs" className="text-xs text-primary hover:underline font-medium" data-testid="link-view-all-txs">
              View all →
            </Link>
          </CardHeader>
          <CardContent className="pt-0 p-0">
            {txsLoading
              ? Array.from({ length: 6 }).map((_, i) => <Skeleton key={i} className="h-14 mx-4 mb-2 rounded-xl" />)
              : txs?.map((tx) => (
                <div key={tx.hash} className="flex items-center gap-3 px-4 py-3 border-b border-border/40 last:border-0 hover:bg-muted/30 transition-colors" data-testid={`scan-tx-row-${tx.hash.slice(0, 8)}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <Link href={`/scan/tx/${tx.hash}`} className="font-mono text-xs text-foreground/80 hover:text-primary transition-colors truncate block" data-testid={`link-scan-tx-${tx.hash.slice(0, 8)}`}>
                      {shortHash(tx.hash)}
                    </Link>
