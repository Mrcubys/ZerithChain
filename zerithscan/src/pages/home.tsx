  ...
  export default function Home() {
    ...
    return (
      <div className="space-y-5">
        {/* Hero Search */}
        <div className="bg-[#21325b] rounded-xl px-6 py-8 text-white">
          <h1 className="text-xl font-semibold mb-1">ZerithScan Blockchain Explorer</h1>
          <p className="text-blue-200 text-sm mb-4">Search the Zerith Chain Mainnet</p>
          <form onSubmit={handleSearch} className="flex gap-2 max-w-2xl">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full h-[42px] pl-10 pr-4 rounded-lg text-[13px] text-gray-900 bg-white border-0 focus:outline-none focus:ring-2 focus:ring-blue-400"
                data-testid="input-hero-search"
              />
            </div>
            <button type="submit" className="h-[42px] px-5 rounded-lg bg-[#3498db] hover:bg-[#2980b9] text-white text-sm font-medium transition-colors" data-testid="button-hero-search">
              Search
            </button>
          </form>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard icon={<Layers className="w-5 h-5 text-blue-500" />} label="Block Height" value={net ? formatNum(net.blockHeight) : "..."} />
          <StatCard icon={<ArrowRightLeft className="w-5 h-5 text-blue-500" />} label="Total Transactions" value={net ? formatNum(net.totalTransactions) : "..."} />
          <StatCard icon={<Users className="w-5 h-5 text-blue-500" />} label="Active Validators" value={net ? String(net.activeValidators) : "..."} />
          <StatCard icon={<Activity className="w-5 h-5 text-blue-500" />} label="Avg Block Time" value={net ? net.averageBlockTime.toFixed(1) + "s" : "..."} />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-100">
              ...
              <Link href="/blocks" className="text-xs text-blue-600 hover:text-blue-800 font-medium flex items-center gap-1" data-testid="link-view-blocks">
            ...
            <div className="divide-y divide-gray-50">
              {blocksLoading ? (
                ...
                <div key={b.height} className="flex items-center gap-4 px-5 py-3 hover:bg-blue-50/30 transition-colors">
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-100">
              ...
              <Link href="/txs" className="text-xs text-blue-600 hover:text-blue-800 font-medium flex items-center gap-1" data-testid="link-view-txs">
                View all <ArrowRight className="w-3 h-3" />
              </Link>
            </div>
            <div className="divide-y divide-gray-50">
              {txsLoading ? (
                ...
                <div key={tx.hash + i} className="flex items-center gap-4 px-5 py-3 hover:bg-blue-50/30 transition-colors">
                  <div className="w-[42px] h-[42px] rounded-lg bg-gray-100 flex items-center justify-center flex-shrink-0">
